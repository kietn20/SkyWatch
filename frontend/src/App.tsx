...
function App() {
    ...
    const [filters, setFilters] = useState<Filters>({
        minAltitude: 0, maxAltitude: 60000, minSpeed: 0, maxSpeed: 1000, country: ''
    });
    ...
    const filteredFlights = useMemo(() => {
        return flights.filter(f => 
            ...
            (filters.country === '' || f.originCountry.toLowerCase().includes(filters.country.toLowerCase()))
        );
    }, [flights, filters]);
    ...
    const handleGeofenceCleared = async () => {
        try {
            await fetch('http://localhost:8080/api/geofence', { method: 'DELETE' });
    ...
    const crossAntimeridianSafeLon = (lon: number) => { return lon > 180 ? lon - 360 : (lon < -180 ? lon + 360 : lon); }
            ...
            <header className="absolute top-0 left-0 right-0 z-[500] p-4 pointer-events-none">
                <div className="max-w-7xl mx-auto flex justify-between items-center bg-slate-900/90 backdrop-blur-md shadow-lg rounded-xl p-4 pointer-events-auto border border-slate-700">
                    <div className="flex items-center gap-3">
                        <Radar className="text-blue-500 animate-[spin_4s_linear_infinite]" size={32} />
                        <div>
                            <h1 className="text-2xl font-black tracking-tight text-white">SkyWatch <span className="text-blue-500">C4ISR</span></h1>
                            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Real-Time Surveillance Dashboard</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-6">
                        <div className="text-right">
                            ...
                            <p className="text-xs font-bold text-slate-400 uppercase">Tracked Targets</p>
                        </div>
